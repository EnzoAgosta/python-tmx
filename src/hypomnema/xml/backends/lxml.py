...
class LxmlBackend(IterableXMLBackend[et._Element]):
  """lxml-based XML backend."""

  def parse(self, path: PathLike[str]) -> et._Element:
    """
  ...
  def write(self, element: et._Element, path: PathLike[str], *, encoding: str = "utf-8") -> None:
    """
  ...
  def iterwrite(
    ...
    *,
    ...
    Root handling:
      - If `root` is specified, it must be an lxml.etree._Element and will
    ...
    Args:
      ...
      elements: An iterable of lxml.etree._Element to write. Ideally this
    ...
    self._iterwrite_core(
      ...
      _tobytes=partial(et.tostring, encoding=encoding, with_tail=True),
    )
  ...
  def iterparse(
    ...
    """
    yield from self._iterparse_core(path, tags, et.iterparse)  # type: ignore
  ...
  def remove(self, parent: et._Element, child: et._Element) -> None:
    parent.remove(child)
