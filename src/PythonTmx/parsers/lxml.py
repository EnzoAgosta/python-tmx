...
class LxmlParser(TmxFileParser):
  """Implementation of TMX file parser using lxml.

  This parser provides a complete implementation of the TmxFileParser interface,
  using lxml for XML processing. It offers both full document parsing with configurable
  traversal strategies and memory-efficient lazy parsing for large files.

  It acts as a drop-in replacement for the StandardParser and has the added benefit
  of providing better performance as well as providing line numbers when encountering
  errors.
      ...
      while stack:
        ...
        if tag not in __TAG_MAP__ and tag not in ("body", "seg"):
          ...
          raise XmlParsingError(
            tag, line=node.sourceline, original_exception=tag_err
          ) from tag_err
        if tag in tag_mask and not yielded_ancestor:
          try:
            ...
            raise XmlParsingError(
              tag, line=node.sourceline, original_exception=e
            ) from e
          continue
