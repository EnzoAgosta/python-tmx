...
class TestLxmlXmlBackendHappy:
  """Tests for the successful execution of LxmlBackend methods."""

  @pytest.fixture(autouse=True)
  def setup(self, mocker):
    self.backend = LxmlBackend()
    self.mocker = mocker
  ...
  def test_create_element_with_bytes_tag(self):
    """Test creating an element with bytes tag."""
    elem = self.backend.create_element(b"root")
    assert self.backend.get_tag(elem) == "root"
  ...
  def test_get_attribute_with_bytes_name(self):
    """Test getting attribute using bytes name."""
    elem = self.backend.create_element("root", attributes={"key": "value"})
    assert self.backend.get_attribute(elem, b"key") == "value"
  ...
  def test_to_bytes_empty_element_has_explicit_closing(self):
    """Test that empty elements have explicit closing tag by default."""
    elem = self.backend.create_element("root")
...
class TestLxmlXmlBackendError:
  """Tests for error conditions in LxmlBackend methods."""
  ...
  def test_append_child_invalid_parent(self):
    ...
    child = self.backend.create_element("child")
    with pytest.raises(TypeError, match="Parent is not an lxml.etree._Element"):
  ...
  def test_append_child_invalid_child(self):
    ...
    parent = self.backend.create_element("parent")
    with pytest.raises(TypeError, match="Child is not an lxml.etree._Element"):
  ...
  def test_get_attribute_invalid_element(self):
    ...
    with pytest.raises(TypeError, match="Element is not an lxml.etree._Element"):
  ...
  def test_set_attribute_bytearray_name(self):
    """Test that bytearray attribute name raises TypeError."""
    elem = self.backend.create_element("root")
    with pytest.raises(TypeError, match="Unexpected bytearray value"):
      self.backend.set_attribute(elem, bytearray(b"key"), "value")
  ...
  def test_iter_children_invalid_tag_filter_type(self):
    """Test that invalid tag filter type raises TypeError."""
    elem = self.backend.create_element("root")
    with pytest.raises(TypeError, match="Unexpected tag filter type"):
      list(self.backend.iter_children(elem, tag_filter=123))
